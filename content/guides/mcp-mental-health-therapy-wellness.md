...
description: "A comprehensive guide to MCP integrations for mental health and wellness — covering FHIR-based EHR servers (health-record-mcp with SMART on FHIR auth, WSO2 fhir-mcp-server, Momentum fhir-mcp-server, xSoVx fhir-mcp with PHI protection and audit logging, Medplum MCP with 33 FHIR tools for SOAP notes and clinical encounters), wearable health platforms (Open Wearables supporting Apple Health Garmin Polar Whoop Suunto Samsung Health Strava with MCP server and deduplication, HealthMCP by Nori), HIPAA compliance frameworks (Innovaccer HMCP with OAuth2 and audit trails, Keragon Healthcare MCP with 300+ integrations, US Compliance MCP for HIPAA CCPA regulations, Secure FHIR MCP with PHI masking), mental health AI tools (Zenify with RAG-powered mood tracking journaling and crisis detection, ChatCBT Obsidian plugin for cognitive behavioral therapy), and the regulatory landscape including FDA digital therapeutics guidance, California SB 243 companion chatbot law, and ethical considerations for AI in mental health care."
content_type: "Guide"
