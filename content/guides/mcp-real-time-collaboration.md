...
description: "A comprehensive guide to 70+ MCP integrations for real-time collaboration — covering chat and messaging servers (Slack official MCP with ~15 tools and OAuth, korotovsky's community Slack server for Apps and GovSlack, Microsoft Work IQ Teams MCP with 24 tools, floriscornel's teams-mcp with 25 tools and KQL search, SaseQ's discord-mcp with 65 tools and full CRUD), collaborative document editing (Notion official MCP server with ~2,390 stars for Markdown-based page and database access, a-bonus's google-docs-mcp with ~176 stars for Google Docs Sheets and Drive, taylorwilsdon's Google Workspace MCP covering Calendar Drive Gmail Docs Sheets Slides Forms Tasks Contacts Chat, Atlassian official remote MCP server with OAuth 2.1 for Jira and Confluence Cloud, sooperset's mcp-atlassian with ~2,600 stars and 72 tools), whiteboarding and diagramming (Miro official AI MCP server with 15 tools and OAuth 2.1, olgasafonova's community Miro server with 88 tools, Excalidraw official MCP, lesleslie's excalidraw-mcp with WebSocket real-time sync, Figma official MCP with design-to-code and write-to-canvas), video conferencing and meetings (tldv MCP server supporting Google Meet Zoom and Microsoft Teams with transcript search and AI highlights, Meeting-BaaS MCP for meeting bots and transcript search, Zoom Transcript MCP, Otter.ai and Fireflies.ai remote MCP servers), project management (Atlassian Jira via official and community servers, Linear official MCP, jerhadf's linear-mcp-server with ~344 stars, Asana official MCP, clickup-mcp-server with natural language task management, Trello and Monday.com community servers), shared workspaces (Airtable official MCP with record queries and permission-aware access, rashidazarang's airtable-mcp with 42 tools, Coda community servers, Google Sheets MCP with full CRUD and OAuth), knowledge management (cyanheads' obsidian-mcp-server with ~299 stars and JsonLogic queries, Roam Research MCP, ProfessionalWiki's MediaWiki MCP Server for any MediaWiki instance), and pair programming (GitHub official MCP server with ~26,400 stars for repos issues PRs and code security, VS Code MCP integration with MCP Apps for interactive UI, CodeRabbit MCP for code reviews). The enterprise collaboration software market is valued at approximately $64.9 billion in 2025, projected to reach $121.5 billion by 2030 at 13.4% CAGR, with AI copilots expected to automate 30-50% of routine collaboration workloads by 2026."
content_type: "Guide"
