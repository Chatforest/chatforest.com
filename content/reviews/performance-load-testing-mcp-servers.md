...
og_description: "Performance & load testing MCP servers: Grafana mcp-k6 (39 stars, v0.6.1 May 11, dynamic docs + k6 v2.0.0), JMeter MCP (65 stars, bottleneck detection), Locust MCP (headless/UI modes), Gatling AI Extensions (JMeter/LoadRunner conversion), Artillery MCP (npm, dry-run validation), Lighthouse MCP (141 stars, SSRF fix), PageSpeed Insights MCP (24 stars, 16 tools). AWS Distributed Load Testing MCP (7 tools), Azure Load Testing MCP (5 tools), NEW canyonlabz/mcp-perf-suite (JMeter + BlazeMeter + Datadog APM correlation). MCPMark (420 stars, ICLR 2026), MCP-Bench (484 stars, NeurIPS 2025). 25+ servers reviewed. Rating: 4.0/5."
content_type: "Review"
