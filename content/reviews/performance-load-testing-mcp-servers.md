...
og_description: "Performance & load testing MCP servers: Grafana mcp-k6 (32 stars, Go, script validation + guided generation + Streamable HTTP), JMeter MCP (63 stars, bottleneck detection), Locust MCP (headless/UI modes), Gatling AI Extensions (84 commits, JMeter/LoadRunner conversion), Artillery MCP (npm, dry-run validation), Lighthouse MCP (135 stars +121%), PageSpeed Insights MCP (16 tools). NEW: AWS Distributed Load Testing MCP (7 tools), Azure Load Testing MCP (5 tools). Plus benchmarking: MCPMark (413 stars, ICLR 2026), MCP-Bench (474 stars, NeurIPS 2025). 25+ servers reviewed. Rating: 4.0/5."
content_type: "Review"
