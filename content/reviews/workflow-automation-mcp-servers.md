...
description: "25+ workflow automation MCP servers compared — n8n (21.1K stars), Zapier, Make, Windmill, Activepieces, Airflow, Prefect, Kestra, Pipedream, and more. Which ones let AI agents actually automate workflows?"
...
content_type: "Review"
card_description: "25+ workflow automation MCP servers compared across n8n, Zapier, Make, Windmill, Activepieces, Airflow, Prefect, Kestra, Pipedream, and Temporal. n8n leads with 21.1K stars. Activepieces now offers ~400 MCP servers. Prefect Horizon adds enterprise MCP infrastructure. Rating: 4.5/5."
...
The headline findings: **n8n dominates adoption with 21,100 stars on its primary MCP server** — the largest MCP server in any automation category, up 13% since April 2026 and 40% since March. **Zapier has 9,000+ apps** with an agentic configuration in rollout. **Activepieces has grown from 280+ to ~400 MCP servers** (the largest open-source MCP toolkit). **Make launched make-skills** — a companion skills package for 40+ AI agents. **Prefect clarified its read-only design** and launched **Prefect Horizon**, a new enterprise MCP server platform covering deploy, registry, gateway, and agents — built on FastMCP. **Windmill reached v1.705.0** (May 20) with continued active development.
...
|--------|-------|----------|-------|-----------|
| [czlonkowski/n8n-mcp](https://github.com/czlonkowski/n8n-mcp) | 21,100 | TypeScript | 20+ | stdio |

**czlonkowski/n8n-mcp** (21,100 stars, TypeScript, MIT) is the primary MCP server for n8n and one of the most starred MCP servers in any category. Stars grew 13% since April 2026 (18,700→21,100) and 40% since March. **v2.51.3 (May 11)** patched a security issue (GHSA-f3rg-xqjj-cj9w) — URL paths and query strings are now redacted in workflow telemetry. **v2.53.2 (May 18)** preserved partial workflow operation order.
...
A second new companion, **czlonkowski/n8n-manager-for-ai-agents**, handles workflow management via the n8n public API directly — full CRUD operations on workflows, execution management, webhook-based execution triggers, and health monitoring. This fills a gap the original n8n-mcp doesn't address: the documentation-and-templates server vs. a workflow management server. The two complement each other.

This is effectively a complete n8n SDK accessible through MCP. The 21,100-star count reflects genuine adoption — n8n itself has 179,000+ stars, and this server is the primary bridge for AI-assisted workflow building.
...
**activepieces/activepieces** (~22,100 stars, TypeScript, MIT) is an open-source automation platform that now exposes **~400 pieces as individual MCP servers** — the largest open-source MCP toolkit available, grown from 280+ since April. When anyone contributes a new piece to Activepieces, it automatically becomes available as an MCP server for Claude Desktop, Cursor, or Windsurf.

The MCP integration is built into the platform rather than being a separate server. Each piece (Google Sheets, OpenAI, Discord, RSS, and 300+ more) gets its own MCP server with API-specific tools. 60% of pieces are community-contributed, giving Activepieces the broadest open-source integration surface.
...
**yangkyeongmo/mcp-server-apache-airflow** (165 stars, Python, MIT) wraps the full Apache Airflow REST API v1. With 60+ operations across DAGs, runs, tasks, variables, connections, pools, datasets, and monitoring, it's the most comprehensive Airflow MCP server.
...
**PrefectHQ/prefect-mcp-server** (~32 stars, Python, Apache 2.0, v0.0.1-beta.10) is the official Prefect MCP server, still in beta with APIs subject to change.

The team has formalized its **read-only design**: the server exposes tools for querying flows, deployments, flow runs, task runs, work pools, events, automations, and logs — with no mutation tools. Write operations (triggering deployments, etc.) are delegated to the Prefect CLI or SDK, often guided by the server's doc lookups. This is a deliberate safety choice: Pro and Enterprise users can restrict service accounts to read-only access via RBAC. Multi-tenant HTTP header auth supports shared instances.

Alongside the MCP server, Prefect launched **Prefect Horizon** — a new enterprise MCP server platform for deploying and managing any MCP servers (not just Prefect). Built on FastMCP (which Prefect claims powers ~70% of MCP servers worldwide, with 64M+ downloads), Horizon covers four capabilities: **Deploy** (push code to GitHub, get a production MCP URL in 60s), **Registry** (catalog all org MCP servers with ownership and versioning), **Gateway** (tool-level RBAC, auth, audit logs), and **Agents** (chat interfaces composed from multiple MCP servers). Free personal tier available. This positions Prefect as MCP infrastructure — not just a workflow tool.

### Windmill (Built-in MCP)

| Server | Stars | Language | Tools | Transport |
|--------|-------|----------|-------|-----------|
| [windmill-labs/windmill](https://github.com/windmill-labs/windmill) | 16,500 | Rust/TS | 5 categories | HTTP Streamable |

**Windmill** (16,500 stars, Rust/TypeScript, AGPLv3) has a **built-in MCP server** — no separate installation needed. Development is highly active: **v1.705.0 released May 20, 2026**, with frequent releases continuing at a fast cadence.
...
**kestra-io/mcp-server-python** (26 stars, Python, Apache 2.0) is the official Kestra MCP server covering 11 tools across the platform: Backfill, Execution, Files, Flow, Key-Value store, Logs, Namespace, Replay, Restart, Resume, and Enterprise Edition tools.
...
|----------|---------------|-------|-----------|-------|----------|
| n8n | czlonkowski/n8n-mcp | 21,100 | Community | 20+ | Node coverage (1,505), templates, +security patch |
| Activepieces | activepieces/activepieces | ~22,100 | Yes | ~400 pieces | Largest open-source MCP toolkit (grew from 280+) |
| Windmill | windmill-labs/windmill | 16,500 | Yes (built-in) | 5 categories | Code-first, v1.705.0 May 20, fast cadence |
| Pipedream | mcp.pipedream.com | — | Yes (hosted) | 10,000+ | 3,000+ API coverage |
| Airflow | yangkyeongmo + astronomer/agents | 165 / ~285 | Mixed | 60+ / 24+ | Deepest tool coverage + AI skills |
| Zapier | zapier/zapier-mcp | 35 | Yes | Dynamic | App breadth (9,000+), agentic in rollout |
| Make | integromat/make-mcp-server | ~155 | Yes | Dynamic | Scenario-as-tool + new make-skills |
| Prefect | PrefectHQ/prefect-mcp-server | ~32 | Yes | 10+ (read-only) | Read-only safety + Horizon enterprise platform |
| Kestra | kestra-io/mcp-server-python | 26 | Yes | 11 | Event-driven, tool disabling |
| Temporal | GethosTheWalrus/temporal-mcp | ~24 | Community | 19 | Dormant since March 27, v1.1.1 last release |
| Dagster | kyryl-opens-ml/mcp-server-dagster | 21 | Community | 9 | Asset materialization (dormant since Apr 2025) |

### What's working

**The low-code side continues its explosive growth.** n8n-mcp reached 21,100 stars (+13% in 26 days, +40% since March). Activepieces grew from 280+ to ~400 MCP servers. The competition between n8n, Zapier, Activepieces, Make, and Pipedream is healthy and driving fast iteration.
...
**Temporal has gone quiet.** GethosTheWalrus/temporal-mcp hasn't had new commits since March 27, 2026 — the "active replacement" for the archived brief-hq server has itself slowed. The Temporal MCP story is weakening.
...
**For low-code automation:** Start with **czlonkowski/n8n-mcp** if you use n8n — it's the most starred automation MCP server by far (21,100 stars, 1,505 nodes, security-patched). Pair it with **n8n-manager-for-ai-agents** for direct workflow management via the n8n API. Use **Zapier MCP** if you need breadth across 9,000+ apps. Use **Activepieces** if you want open-source Zapier-like breadth with ~400 MCP-enabled pieces. Use **Make MCP** plus the new **make-skills** package if you've already built complex multi-step scenarios.
...
**For enterprise MCP infrastructure:** **Prefect Horizon** (deploy, registry, gateway, agents) is worth watching if you're managing multiple MCP servers at scale. Built on FastMCP with a free personal tier.

**Rating: 4.5/5** — The workflow automation MCP category remains one of the most mature and well-served. n8n-mcp's continued star growth (21,100, +40% since March), Activepieces reaching ~400 MCP servers, Make launching make-skills, Windmill's active v1.705.0 cadence, and Prefect's pivot to MCP infrastructure with Horizon are all meaningful advances since April. The Temporal slowdown and continued Dagster stagnation are negatives. Rating held at 4.5/5.

*This review was refreshed on 2026-05-21 using Claude Sonnet 4.6 (Anthropic).*
