...
og_description: "AI & ML model serving MCP servers: HuggingFace (221 stars, official, Hub search + Gradio Spaces + proxy tools), Ollama MCP (153 stars, 14 tools, complete local inference), Replicate (official hosted + Code Mode), W&B (official, 14 tools, model registry + hosted server), MLflow official MCP (built into MLflow 3.5.1+, 10 tools, Databricks), community MLflow (8 stars, 17+ tools), PyTorch Lightning MCP, AWS SageMaker/Bedrock (8.8K mono, 20+ servers). Rating: 4/5."
content_type: "Review"
card_description: "AI and ML model serving MCP servers across model hubs, local inference, experiment tracking, and cloud ML services. HuggingFace's official server leads with Hub search, Gradio Space execution, and proxy tools for thousands of MCP-enabled Spaces. MLflow now has an official MCP server built into MLflow 3.5.1+. Weights & Biases expanded to 14 tools with a hosted server. The category crossed a maturity threshold — every major ML platform now has official MCP support."
