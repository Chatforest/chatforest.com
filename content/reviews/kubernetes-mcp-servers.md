...
description: "Kubernetes has a strong MCP ecosystem with two leading servers above 1,000 stars: Flux159/mcp-server-kubernetes (1,400 stars, TypeScript, 20+ tools) and Red Hat's containers/kubernetes-mcp-server (1,500 stars, Go). AWS MCP Server went GA (May 2026). Lens Desktop ships built-in MCP. Rating: 4/5."
og_description: "Kubernetes MCP ecosystem: two 1,000+ star servers, Helm integration, OpenShift support, read-only modes. Lens Desktop built-in MCP (1M+ users, March 2026). AWS MCP Server GA (May 2026). kagent CNCF Sandbox (2,700 stars). No official CNCF server but Red Hat leads. Rating: 4/5."
content_type: "Review"
...
**At a glance:** Kubernetes has **no official MCP server** from CNCF or the Kubernetes project, but the community has built a strong ecosystem led by two servers above 1,000 stars. [Flux159/mcp-server-kubernetes](https://github.com/Flux159/mcp-server-kubernetes) (~1,400 stars, TypeScript, MIT, 20+ tools) provides kubectl-style operations with Helm support — **CVE-2026-39884 (High) argument injection patched in v3.5.0**. [containers/kubernetes-mcp-server](https://github.com/containers/kubernetes-mcp-server) (1,500 stars, Go, Apache-2.0) from Red Hat leads with Tekton integration and Microsoft Entra ID auth in v0.0.61. **NEW: [kagent](https://github.com/kagent-dev/kagent) (2,700 stars, CNCF Sandbox)** — a Kubernetes-native agentic AI framework with pre-built MCP servers for K8s, Istio, Helm, Argo, Prometheus, Grafana, and Cilium. **KubeCon EU 2026 held the first Agentics Day** — a dedicated co-located event for MCP + agents on Kubernetes. **Google GKE Agent Sandbox** now provides managed remote MCP server (GA) with sub-second sandbox latency. **NEW: Lens Desktop** (Mirantis, 1M+ users) launched a built-in MCP server (March 18, 2026) — the first major K8s management tool to ship one. **AWS MCP Server went GA** (May 6, 2026) covering all AWS services including EKS/ECS. CNCF **nearly doubled certified K8s AI platforms** and added **agentic workflow validation** to certification. With **5.6 million Kubernetes developers** and **82% of container users running K8s in production**, the MCP ecosystem matches the platform's importance. This is the **fifth review in our [Developer Tools MCP category](/reviews/ide-code-editor-mcp-servers/)**. Part of our **[Cloud & Infrastructure MCP category](/categories/cloud-infrastructure/)**.
...
**kagent** ([2,700 stars](https://github.com/kagent-dev/kagent), Go/TypeScript/Python, Apache-2.0, **CNCF Sandbox**, v0.9.0 April 2026) — Kubernetes-native agentic AI framework from Solo.io, now a **CNCF Sandbox project**. Ships with **pre-built MCP servers** for Kubernetes, Istio, Helm, Argo, Prometheus, Grafana, and Cilium. Defines agents as **Kubernetes CRDs** with declarative YAML configuration. Supports multiple LLM providers (OpenAI, Anthropic, Google Vertex AI, Ollama). Built on Agent2Agent (A2A) protocol, Agent Development Kit (ADK), and MCP. Includes web UI, CLI, and engine components. **139 releases** indicate rapid iteration. The most significant new entry in the K8s MCP ecosystem — not just an MCP server, but a full framework for running AI agents *on* Kubernetes with MCP as the tool interface.

**Google GKE Agent Sandbox** (announced KubeCon EU 2026, **managed remote MCP server now GA**) — Secure agentic AI workloads on GKE with kernel-level isolation (gVisor). **Sub-second latency** for fully isolated agent workloads — up to 90% improvement over cold starts. Managed remote MCP server enables AI agents to interact with K8s clusters without local tooling. Use cases: executing untrusted LLM-generated code, persistent cloud development environments, stateful single-pod services. Part of Google's broader push at Next '26 including GKE hypercluster and intent-based autoscaling.

**Akuity/argocd-mcp** ([416 stars](https://github.com/akuity/argocd-mcp), TypeScript, Apache-2.0, v0.6.0 March 2026) — MCP server for Argo CD from the creators of Argo. Application management (create, read, update, delete, sync), resource tree visualization, workload logging. Read-only mode available. Supports stdio and HTTP stream transport. Stateless operation supports Kubernetes HPA deployments. Presented at **KubeCon EU 2026 Agentics Day** with Intuit. Also available under [argoproj-labs/mcp-for-argocd](https://github.com/argoproj-labs/mcp-for-argocd).

**Lens Desktop MCP** (Mirantis, launched March 18, 2026) — Built-in MCP server in **Lens Desktop**, the Kubernetes IDE with 1 million+ users. The **first major Kubernetes management tool to ship a built-in MCP server**. Works with Claude Code, ChatGPT, Cursor, GitHub Copilot, and any MCP-compatible AI assistant. Provides cluster context and connectivity directly to coding assistants — no kubeconfig juggling inside AI tools. Available in the latest Lens Desktop at [lenshq.io](https://lenshq.io/). Lens is the natural AI entry point for organizations already using it as their primary K8s IDE.

**AWS MCP Server (GA, May 6, 2026)** — The broader [AWS MCP Server](https://github.com/awslabs/mcp) from AWS Labs reached general availability on May 6, 2026. Covers all AWS services via a single tool including EKS, ECS, EC2, Lambda, and more. IAM-based guardrails, Amazon CloudWatch metrics, and AWS CloudTrail audit logging. Sandboxed Python script execution for multi-step operations without local filesystem access. No additional charge — you pay only for AWS resources used. The **fully managed Amazon EKS MCP Server** (cloud-hosted, per the preview blog post) remains separately in preview. Google's GKE managed MCP server reached GA first; AWS answered with the broader AWS MCP Server GA.
...
1. **No official CNCF/Kubernetes MCP server — but kagent is closest** — Unlike GitHub (official server with 28.2k stars) or Docker (MCP Gateway, MCP Catalog), there is no CNCF-maintained MCP server for Kubernetes directly. However, **kagent (CNCF Sandbox, 2,700 stars)** ships with pre-built MCP servers for Kubernetes and 6 other cloud-native tools — making it the closest thing to a CNCF-blessed K8s MCP interface. Red Hat's server under the `containers` org remains the leading standalone option. CNCF held **Agentics Day at KubeCon EU 2026** — a dedicated half-day event for MCP + agents — and **nearly doubled certified K8s AI platforms** with agentic workflow validation. The ecosystem is converging but still lacks a single canonical K8s MCP server.
...
7. **Multi-cluster management gap narrowing further** — All top servers support multiple clusters via kubeconfig contexts, but standalone MCP servers still provide one-context-at-a-time access. SUSE Rancher Prime's built-in MCP server and mrostamii/rancher-mcp-server with Fleet GitOps provide fleet-level visibility. **NEW: kagent defines agents as Kubernetes CRDs** — enabling multi-cluster agent deployment natively within the K8s control plane. **Google GKE Agent Sandbox** provides managed multi-cluster MCP with sub-second sandbox provisioning. For organizations running 6+ production clusters, there are now multiple viable options: Rancher for fleet management, kagent for Kubernetes-native agent orchestration, and GKE Agent Sandbox for Google Cloud users.
...
9. **Cloud provider authentication improving — both Google and AWS now GA** — Most servers rely on kubeconfig, which works for local development but creates friction for managed K8s services. **Red Hat's v0.0.61 adds Microsoft Entra ID support** with On-Behalf-Of token exchange — the first K8s MCP server with Azure AD integration. alexei-led's server supports AWS/GKE/AKS credentials natively. **Google's GKE managed remote MCP server is now GA** — the first major cloud provider to ship a production-ready managed K8s MCP endpoint. **AWS answered on May 6, 2026** with the broader AWS MCP Server reaching GA — covering EKS, ECS, and all AWS services via IAM-based guardrails and CloudTrail audit logging. The fully managed EKS-specific server (separate preview offering) remains in preview. The cloud authentication gap has narrowed significantly with Google and AWS both offering GA managed paths.
...
Kubernetes has a **strong and maturing MCP ecosystem** that is now expanding beyond standalone servers into platform-level integration. The core servers are stable: Red Hat's containers/kubernetes-mcp-server holds at 1,500 stars with v0.0.61, Flux159/mcp-server-kubernetes at ~1,400 stars with v3.5.0. The biggest development this cycle is **ecosystem expansion**: **kagent (CNCF Sandbox, 2,700 stars)** brings Kubernetes-native agentic AI with pre-built MCP servers for K8s + 6 cloud-native tools. **Google's GKE Agent Sandbox** provides managed remote MCP (now GA) with sub-second sandbox latency. **Lens Desktop** (Mirantis, 1M+ users) shipped a built-in MCP server on March 18, 2026 — the first major Kubernetes IDE to do so. **AWS MCP Server reached GA** (May 6, 2026) covering all AWS services including EKS/ECS, with IAM guardrails and CloudTrail audit logging. **KubeCon EU 2026 held the first Agentics Day** — a dedicated co-located event where MCP appeared across every track, with Linkerd presenting MCP routing support (first service mesh), Akuity/Intuit presenting an Argo CD MCP server (416 stars), and CNCF **nearly doubling certified K8s AI platforms** with agentic workflow validation.

The **4/5 rating holds** — the core server landscape is largely unchanged: Red Hat and Flux159 remain the leaders. strowk remains dormant (7+ months). The new additions (Lens Desktop MCP, AWS MCP Server GA) represent the ecosystem maturing at the platform layer — AI assistants can now reach Kubernetes through IDEs (Lens), cloud managed endpoints (GKE, AWS), and the open-source leader servers. An official CNCF K8s MCP server is still absent, though kagent (CNCF Sandbox) partially fills that gap.
...
- **Enterprise fleet operators** — SUSE Rancher Prime's built-in MCP, mrostamii/rancher-mcp-server with Fleet GitOps, and **NEW: kagent** with Kubernetes CRD-based agent definitions provide fleet-level AI capabilities
- **Google Cloud users** — **NEW: GKE Agent Sandbox** provides managed remote MCP (GA) with sub-second isolation — zero local setup required
- **AWS users** — **AWS MCP Server (GA, May 2026)** covers EKS and all AWS services with IAM guardrails and CloudTrail auditing — no third-party tools required
- **Lens Desktop users** — **Built-in MCP server** (March 2026) connects the AI coding assistant of your choice to your clusters directly from the IDE you already use
- **Cloud-native AI builders** — **NEW: kagent (CNCF Sandbox)** offers a full framework for deploying and managing AI agents as Kubernetes workloads with MCP tool integration
...
- **Teams expecting stability** — All standalone servers are pre-1.0. Red Hat's v0.0.60 introduced a breaking change (user-scoped targets). Pin versions in production configurations
