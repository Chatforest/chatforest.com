...
**AWS EKS/ECS MCP** (still in preview) — Fully managed cloud-hosted MCP servers from AWS. IAM integration, CloudTrail audit logging. Not open-source. No GA announcement as of May 2026. AWS predicts 2026 as "the year of agentic workloads in production on EKS." **Google's GKE managed MCP server reaching GA first** puts competitive pressure on AWS to announce a timeline.
...
9. **Cloud provider authentication improving — Google leads** — Most servers rely on kubeconfig, which works for local development but creates friction for managed K8s services. **Red Hat's v0.0.61 adds Microsoft Entra ID support** with On-Behalf-Of token exchange — the first K8s MCP server with Azure AD integration. alexei-led's server supports AWS/GKE/AKS credentials natively. **Google's GKE managed remote MCP server is now GA** — the first major cloud provider to ship a production-ready managed K8s MCP endpoint. AWS EKS MCP remains in preview (no GA as of May 2026). The authentication gap is narrowing, with Google now ahead.
...
Kubernetes has a **strong and maturing MCP ecosystem** that is now expanding beyond standalone servers into platform-level integration. The core servers are stable: Red Hat's containers/kubernetes-mcp-server holds at 1,500 stars with v0.0.61, Flux159/mcp-server-kubernetes at ~1,400 stars with v3.5.0. The biggest development this cycle is **ecosystem expansion**: **kagent (CNCF Sandbox, 2,700 stars)** brings Kubernetes-native agentic AI with pre-built MCP servers for K8s + 6 cloud-native tools. **Google's GKE Agent Sandbox** provides managed remote MCP (now GA) with sub-second sandbox latency — the **first major cloud provider with a GA managed K8s MCP endpoint**. **KubeCon EU 2026 held the first Agentics Day** — a dedicated co-located event where MCP appeared across every track, with Linkerd presenting MCP routing support (first service mesh), Akuity/Intuit presenting an Argo CD MCP server (416 stars), and CNCF **nearly doubling certified K8s AI platforms** with agentic workflow validation.

The **4/5 rating holds** — kagent and GKE Agent Sandbox are significant additions, but the core server landscape is largely unchanged: Red Hat and Flux159 remain the leaders with no new releases this cycle. strowk remains dormant (7+ months). The ecosystem's center of gravity is shifting from "build MCP servers for K8s" to "run AI agents on K8s using MCP" — kagent and GKE Agent Sandbox represent this next phase. An official CNCF K8s MCP server is still absent, though kagent (CNCF Sandbox) partially fills that gap.
