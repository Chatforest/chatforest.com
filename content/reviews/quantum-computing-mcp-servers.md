...
**IBM is the only major quantum computing vendor with official MCP server support.** The [Qiskit/mcp-servers](https://github.com/Qiskit/mcp-servers) repository (28 stars, Apache 2.0, Python, 752 commits) contains **five MCP servers** — four official from IBM and one community contribution — covering the full quantum development workflow. Install all via `pip install "qiskit-mcp-servers[all]"`.

**Qiskit MCP Server** provides core circuit creation, manipulation, transpilation, and serialization (OpenQASM 3 and QPY formats) for local development. No authentication required. **Qiskit Runtime** connects to IBM Quantum cloud services for backend discovery and job submission to real hardware. **Qiskit IBM Transpiler** offers AI-powered circuit optimization using advanced routing and optimization algorithms. **Qiskit Docs** provides searchable access to the complete Qiskit documentation ecosystem — the only server requiring no authentication at all.

The community-contributed **Qiskit Gym** adds reinforcement learning-based quantum circuit synthesis, documented in detail by IBM Quantum researcher Juan Cruz-Benito in a January 2026 post. AI agents can train RL models (PPO or AlphaZero) asynchronously for three synthesis types: permutation synthesis (SWAP gate routing), LinearFunction synthesis (CNOT circuits for boolean functions), and Clifford synthesis (H, S, and CNOT gates). Available on PyPI.

All servers are built on the **FastMCP** framework with async-first design, each distributed as an independent PyPI package. Python 3.10+ required (3.11+ recommended). Registered in the MCP Registry for automatic discovery.

**Q1 2026 Qiskit ecosystem updates** add context for MCP workflows: **Qiskit v2.4** brings new C-API features, improved Python extension support, and faster fault-tolerant compilation. **Qiskit Fermions** — a new quantum chemistry library — extends Qiskit with fermionic mappers, operator tools, and an extensible circuit-synthesis library for molecular simulation. **Concurrent experiments** (up to 4 parallel runs) via Qiskit Functions let agents pipeline quantum workflows more efficiently.

**IBM hardware context (June 2026):** IBM now offers two QPU families accessible via the MCP servers. **Heron r3** delivers ~2.15×10⁻³ two-qubit error rate across 100 qubits, with dozens of qubit pairs below the 10⁻³ threshold. **Nighthawk** (available January 2026) is IBM's new 120-qubit processor in a square-lattice topology with 218 tunable couplers, supporting up to 5,000 two-qubit gates and a median T1 coherence time of ~350 microseconds — IBM's longest yet. Future Nighthawk iterations target 7,500 two-qubit gates by end-2026 and 10,000 in 2027.
...
The highest-starred project in the quantum MCP space (42 stars) — but it's a **client**, not a server. Provides quantum-safe communication for MCP interactions using **Kyber-768 KEM** (key encapsulation) and **SPHINCS+** (digital signatures) with **AES-256-GCM** encrypted payloads. Establishes secure sessions via KEM handshake, encrypts all request/response traffic, and verifies digital signatures. CLI commands for key generation, inference requests, agent workflows, and policy updates. Includes mock FastAPI server for testing.

Recent updates added four new diagnostic CLI commands: `validate-config` (environment and configuration validation), `inspect-keys` (key status reporting), `test-connection` (connectivity testing without running operations), and `benchmark` (performance measurement for quantum-safe operations). An automated installation script now handles complex dependencies, and a comprehensive `TROUBLESHOOTING.md` was added for onboarding.
