---
title: "Claude Code GitHub Action Had a Supply Chain Flaw: What Happened, What's Fixed, and How to Harden Your CI/CD"
date: 2026-06-09
description: "A prompt injection + permission bypass in the official Claude Code GitHub Action let unauthenticated attackers hijack any repo using it. Patched in v1.0.94. Here is the attack chain, the broader pattern, and a hardening checklist for builders running Claude in CI/CD."
og_description: "Security researcher RyotaK found that Claude Code's GitHub Action would unconditionally trust any actor ending in [bot], letting attackers use a free GitHub App token to inject prompts via issues and steal CI/CD secrets. Patched. But the class of vulnerability — prompt injection in agentic CI/CD — is not going away. Here is what builders need to do."
content_type: "Builder Guide"
card_description: "The official Claude Code GitHub Action had a critical flaw: the checkWritePermissions function trusted any actor ending in [bot] regardless of actual permissions. An unauthenticated attacker with a GitHub App installation token could create a malicious issue, inject prompts into Claude's context, and escalate to full repo compromise including OIDC token theft. Patched in v1.0.94 (CVSS 4.0: 7.8). Researcher RyotaK of GMO Flatt Security has now identified approximately 50 ways to break Claude Code's permission model. This is a class of vulnerability, not a single bug."
tags: ["claude-code", "github-actions", "security", "prompt-injection", "supply-chain", "ci-cd", "anthropic", "developer-tools", "agentic-ai", "oidc", "secrets-management"]
