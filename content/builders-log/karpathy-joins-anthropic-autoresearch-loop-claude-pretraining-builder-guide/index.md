---
title: "Karpathy Joins Anthropic: The AutoResearch Loop and What It Means for Builders"
date: 2026-05-19
description: "Andrej Karpathy joined Anthropic's pre-training team on May 19 to run the loop he proved in March: AI agents designing, running, and evaluating training experiments, compressing the research cycle on future Claude models. Here's what the Karpathy Loop is, how it works, and what it signals for builders building on Claude."
og_description: "Karpathy's AutoResearch project: 630 lines, 700 experiments, 11% training speedup in 2 days. Now it's inside Anthropic's Claude pre-training pipeline. Builder implications: eval loops, multi-model pipelines, and accelerating model velocity."
content_type: "Builder's Log"
categories: ["Anthropic", "AI Infrastructure", "Developer Tools"]
tags: ["anthropic", "karpathy", "autoresearch", "pretraining", "claude", "eval-loops", "self-improving-ai", "model-velocity", "ai-research"]
---
...
1. **Generate**: The current model (or agent) produces a candidate change — a code modification, a dataset filter rule, a hyperparameter setting
2. **Evaluate**: A model (the same or a stronger one) assesses whether the candidate improved the target metric
3. **Filter**: Improvements are kept; failures are discarded
4. **Train**: The next model trains on the improved setup — and becomes the generator for the next cycle
