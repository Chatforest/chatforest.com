...
**June 14 update:** The ruling remains pending — no Ninth Circuit opinion has issued as of this writing. The ACLU filed an amicus brief in the case, signaling that civil liberties organizations view the authorization question as having implications well beyond commercial scraping. The ACLU's entry is notable: the CFAA's scope question — whether a platform's terms-of-service revocation can override a user's delegation of access — touches directly on user autonomy and the right to use software tools of one's choosing. We continue to watch for the opinion; [blog.ericgoldman.org](https://blog.ericgoldman.org) is the best source for a post-argument recap when it publishes.
