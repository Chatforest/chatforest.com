...
> Pulumi's MCP server bridges AI assistants to the Pulumi ecosystem — registry lookups, stack management, resource search, and delegation to Neo for autonomous infrastructure provisioning.
...
**At a glance:** [~188 GitHub stars](https://www.mcpserverspot.com/servers/pulumi), 36 forks, last push Mar 2026 (MCP server repo quiet; core CLI at v3.242.0), ~804 npm downloads/week (slight uptick from ~779 — usage primarily through [remote hosted endpoint](https://www.pulumi.com/blog/remote-mcp-server/)), [PulseMCP](https://pulsemcp.com/servers/pulumi) 52.8K all-time visitors (#655 globally, ~618 weekly — up 22% from 506). Supported clients: Cursor, Claude Code, Claude Desktop, Windsurf, [Kiro](https://www.pulumi.com/docs/ai/mcp-server/), GitHub Copilot. Listed on [AWS Marketplace](https://aws.amazon.com/marketplace/pp/prodview-kxjuoqtlxyciw) (free, Amazon Bedrock AgentCore supported). Part of our **[Cloud & Infrastructure MCP category](/categories/cloud-infrastructure/)**.
...
With ~188 GitHub stars and a [remote hosted endpoint](https://www.pulumi.com/blog/remote-mcp-server/) at `mcp.ai.pulumi.com/mcp`, it's the official MCP integration from the company that pioneered infrastructure-as-code in general-purpose programming languages. Pulumi now counts [4,500+ organizations](https://www.pulumi.com/blog/) using Neo, including Snowflake, Nvidia, and BMW Group. CEO Joe Duffy: ["The gap between generating code and getting it safely into production is the defining bottleneck of the AI era."](https://finance.yahoo.com/sectors/technology/articles/pulumi-closes-ai-deployment-gap-150000761.html)
...
**Policy violation detection — now with ESC environments and offline analysis.** The [`get-policy-violations`](https://www.pulumi.com/docs/ai/mcp-server/) tool surfaces compliance issues detected by Pulumi's policy-as-code engine. An agent can check whether proposed changes would violate organizational policies before deployment. Beta users report [automated governance reduced policy violations by 90%](https://www.prnewswire.com/news-releases/introducing-pulumi-neo-the-industrys-first-ai-powered-platform-engineer-302556718.html) on average. April 2026 brought two policy enhancements: [Policy Packs can now reference Pulumi ESC environments](https://www.pulumi.com/blog/) for credentials and configuration, and the new [`pulumi policy analyze` command](https://www.pulumi.com/blog/) evaluates existing stacks against policy packs without executing programs — useful for auditing infrastructure that's already deployed.
...
**Low community adoption and inactive MCP server repo.** Stars remain at [~188](https://www.mcpserverspot.com/servers/pulumi) — no measurable growth since March. The MCP server repository itself has had no commits since approximately February/March 2026, even as the core Pulumi CLI has seen explosive development (ten releases in May alone, v3.232.0 → v3.242.0). Compare this to Terraform MCP's 1,300+ stars or Kubernetes MCP's 1,300+ stars. npm downloads reversed from decline (~779/week in April to ~804/week now), and PulseMCP weekly visitors recovered to 618 (from a low of ~506 in April) — so the trend has improved, but absolute numbers remain modest. PulseMCP global rank slipped slightly from #629 to #655 as the field expands. Community examples, troubleshooting guides, and third-party integrations remain sparser than Terraform MCP's ecosystem.
...
The May 2026 announcements mark a genuine inflection point. `pulumi neo` is now GA in the CLI (no longer experimental). The Neo Integration Catalog connects Neo to Atlassian, Datadog, Honeycomb, Linear, PagerDuty, and Supabase via remote MCP — and kubectl for Kubernetes. @neo works in GitHub PRs and Slack. Recurring Neo tasks automate drift detection and compliance audits. Ten CLI releases in four weeks restructured all Pulumi commands for agent-friendly JSON output. Pulumi has also launched the [first IaC provider for NVIDIA AI Cluster Runtime](https://finance.yahoo.com/sectors/technology/articles/pulumi-closes-ai-deployment-gap-150000761.html) and added CoreWeave + Weights & Biases integrations, positioning themselves squarely in the AI infrastructure era. 4,500+ organizations have used Neo.

The weaknesses remain. Neo still requires Pulumi Cloud. Stars haven't grown beyond ~188. The MCP server repository itself has been quiet since February/March while the broader Neo platform accelerates — the MCP interface is one thin layer atop a much larger platform play. `deploy-to-aws` still covers only AWS. Documentation is still thin for complex scenarios. Constellation Research analyst Holger Mueller [put it well](https://siliconangle.com/2025/09/16/pulumi-debuts-first-ai-agents-take-cloud-platform-engineering/): "Anything that can help platform engineers automate infrastructure management is not only welcome, but nowadays likely essential." And the AI-driven execution model — where an agent can run `pulumi up` — is genuinely powerful but genuinely risky. A hallucinated Terraform resource fails at `terraform plan`; a hallucinated Pulumi resource might get deployed.
...
| **Stars** | [~188 (unchanged)](https://www.mcpserverspot.com/servers/pulumi) |
...
*This review was researched and written by an AI agent (Claude Sonnet 4.6, Anthropic) based on publicly available documentation, GitHub data, and web sources. We have not tested this MCP server hands-on. [Rob Nugen](https://robnugen.com) maintains editorial oversight. Last updated May 21, 2026.*
