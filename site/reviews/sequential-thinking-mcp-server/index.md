...
**At a glance:** [83,500+ stars](https://github.com/modelcontextprotocol/servers/stargazers) (monorepo), [~105K weekly npm downloads](https://www.npmjs.com/package/@modelcontextprotocol/server-sequential-thinking) (rebounding after Jan-Feb dip), [v2025.12.18](https://registry.npmjs.org/@modelcontextprotocol/server-sequential-thinking/latest), 1 tool, Apache 2.0 license, ~66.5K weekly PulseMCP visitors (#10 globally, ~4.1M all-time).
...
**Downloads rebounded in March.** Monthly npm downloads dipped in January–February 2026 ([328K in January, 298K in February](https://www.npmjs.com/package/@modelcontextprotocol/server-sequential-thinking)) but surged back to [411K in March](https://api.npmjs.org/downloads/range/2026-03-01:2026-03-31/@modelcontextprotocol/server-sequential-thinking) — exceeding the December 2025 peak. [Weekly downloads now stand at ~105K](https://api.npmjs.org/downloads/point/last-week/@modelcontextprotocol/server-sequential-thinking) (as of April 10, 2026). The earlier "declining" narrative appears premature; the Jan-Feb dip was seasonal rather than structural.
...
**Memory leak fix proposed but not merged.** [PR #3321](https://github.com/modelcontextprotocol/servers/pull/3321) (opened February 11, 2026, still open as of April 2026) documents that the `thoughtHistory` and `branches` arrays grow without bound — in sessions running 6-8+ hours, RAM consumption can hit 10GB+. The PR proposes a configurable history limit (default 1000 thoughts) via `SEQUENTIAL_THINKING_MAX_HISTORY` and a `clearHistory()` method, but has not been merged.
...
**Better type coercion.** [PR #3533](https://github.com/modelcontextprotocol/servers/pull/3533) (merged March 15, 2026) switched to `z.coerce` for number parameters and a custom preprocessor for booleans, fixing the long-standing issue where LLMs send `"1"` instead of `1`. The boolean handling required special care — `z.coerce.boolean()` incorrectly converts `"false"` to `true`, so a custom function was used instead.

**License transitioned to Apache 2.0.** The `modelcontextprotocol/servers` monorepo now uses [Apache 2.0 for new code](https://github.com/modelcontextprotocol/servers/blob/main/LICENSE), with existing MIT contributions preserved and documentation under CC-BY-4.0. Functionally similar to MIT for most users, but worth noting for compliance teams.

**No new npm release since December 2025.** The latest published version remains [v2025.12.18](https://registry.npmjs.org/@modelcontextprotocol/server-sequential-thinking/latest). The repo has advanced to v0.6.2 internally, but the March 2026 fixes (type coercion, tool annotations) are not yet released to npm.
...
**Still maintained in the official monorepo.** Unlike Puppeteer and SQLite (moved to servers-archived), Sequential Thinking remains in the active [`modelcontextprotocol/servers`](https://github.com/modelcontextprotocol/servers) repo ([83,500+ stars](https://github.com/modelcontextprotocol/servers/stargazers)). It received [two fixes in March 2026](https://github.com/modelcontextprotocol/servers/pull/3534) alone (type coercion, tool annotations). The license transitioned to Apache 2.0 in January 2026. Version [2025.12.18](https://registry.npmjs.org/@modelcontextprotocol/server-sequential-thinking/latest) is the latest npm release.
...
**The `totalThoughts` parameter is uncontrollable by default.** Users tried command-line arguments, environment variables, and configuration options to constrain thought counts ([#2226](https://github.com/modelcontextprotocol/servers/issues/2226) — still open). The `MAX_TOTAL_THOUGHTS` environment variable was eventually added as a workaround, but by default there's no limit — an agent can generate dozens of thought steps, consuming tokens and time without constraint. The model decides how many thoughts to use, and it often decides on more than necessary.
...
The Sequential Thinking MCP server introduced an important idea: making AI reasoning structured, visible, and controllable. The branching model is elegant, the thought revision concept is sound, and the zero-dependency setup is as simple as MCP servers get. The picture is more nuanced than a simple decline story. Downloads dipped in January–February 2026 but [rebounded to 411K in March](https://api.npmjs.org/downloads/range/2026-03-01:2026-03-31/@modelcontextprotocol/server-sequential-thinking) — exceeding the December 2025 peak — and [weekly downloads now stand at ~105K](https://api.npmjs.org/downloads/point/last-week/@modelcontextprotocol/server-sequential-thinking). Anthropic explicitly recommends extended thinking "instead of a dedicated think tool in most cases," yet usage keeps growing. A [memory leak in long-running sessions](https://github.com/modelcontextprotocol/servers/pull/3321) remains unpatched. Agents still rarely use the branching and revision features that justify this over simpler approaches. It's still maintained — it received [two fixes in March 2026](https://github.com/modelcontextprotocol/servers/pull/3534), and hasn't been archived like Puppeteer or SQLite — so it's not abandoned. At [~105,000 weekly downloads](https://api.npmjs.org/downloads/point/last-week/@modelcontextprotocol/server-sequential-thinking) and #10 on PulseMCP, it clearly has users. For auditable reasoning traces, MCP clients without extended thinking support, or debugging agent decision-making, it remains the right choice. But its creators are pointing users elsewhere, even as the community keeps downloading it.
{{< /verdict >}}
...
*This review was last updated on 2026-04-12 using Claude Opus 4.6 (Anthropic).*
